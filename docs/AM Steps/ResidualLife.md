...
- \( ESAL8 \) = Accumulated Equivalent Single Axle Loads (ESALs) after 8 years. ESALs are standardize traffic loads, expressed in the number of 18,000-pound single axle loads .
...
Note that this is a very simple equation, that assumes the same number of breaks would happen each year for a group of pipes (e.g. all cast-iron pipes in the system) and no other factors are accounted for. 
...
Thus, the equation \( N = k \cdot Age \) becomes:
